...
import { buildAdjacency, computeHighlight, edgeId, EMPTY_HIGHLIGHT } from './highlight'
...
export default function App() {
  ...
  const { flow, reasoning_trail: trail, decisions, quality, project_meta: meta } = simulation
  ...
  const nodes: Node<CardData>[] = useMemo(
    () =>
      layout(visible).map((node) => ({
        ...
        data: {
          node,
          dimmed: hovered !== null && !highlight.nodes.has(node.id),
          highlighted: highlight.nodes.has(node.id),
          direct: highlight.direct.has(node.id) || node.id === hovered,
          selected: node.id === selected,
        },
      })),
    [visible, hovered, highlight, selected],
  )
  ...
  const counts = useMemo(() => {
    ...
    for (const node of flow.nodes) byKind.set(node.kind, (byKind.get(node.kind) ?? 0) + 1)
    return byKind
  }, [flow.nodes])
      ...
      <header className="topbar">
        <div>
          ...
          <p className="sub">
            ...
            {String(meta.it_load_mw ?? '')} MW · {String(meta.redundancy_topology ?? '')}
          </p>
        </div>
        <div className="badges">
          ...
          {Boolean(quality.export_blocked) && (
            ...
            >
              Export blocked — {String(quality.tier_1_count ?? 0)} Tier-1 unsigned
            </span>
          )}
          {openForks > 0 && (
            <span className="badge badge-alarm">{openForks} open decision point(s)</span>
          )}
          {!quality.governance_complete && (
            <span className="badge badge-warn">
              Governance incomplete · {String(quality.tier_2_count ?? 0)} on unverified data
            </span>
          )}
          <span className="badge mono">lib {String(meta.library_version ?? '')}</span>
        </div>
      </header>
        ...
        <nav className="sidebar">
              ...
              {KIND_ORDER.map((kind) => {
                ...
                return (
                  <li key={kind}>
                    ...
                    >
                      <span className="swatch" style={{ background: style.accent }} />
                      <span className="legend-label">
                        {style.glyph} {style.label}
                      </span>
                      <span className="count mono">{counts.get(kind) ?? 0}</span>
                    </button>
                  </li>
                )
              })}
          ...
          <section>
            ...
            <p className="small muted">
              Collapse a stage to keep a large graph navigable — the spec's rule is never to
              render everything flat.
            </p>
            <ul className="legend">
              {stages.map((stage) => (
                <li key={stage}>
                  <button
                    className={hiddenStages.has(stage) ? 'legend-item is-off' : 'legend-item'}
                    onClick={() => setHiddenStages((s) => toggle(s, stage))}
                  >
                    <span className="legend-label">{stage.replace(/_/g, ' ')}</span>
          ...
          <section>
            ...
            <ul className="legend legend-static">
              {Object.entries(EDGE_STYLES).map(([kind, style]) => (
                <li key={kind}>
                  <span className="edge-swatch" style={{ background: style.stroke }} />
        ...
        <main className="canvas" data-testid="canvas">
          ...
          >
            ...
            <MiniMap
              pannable
              zoomable
              nodeColor={(n) => KIND_STYLES[(n.data as CardData).node.kind].accent}
